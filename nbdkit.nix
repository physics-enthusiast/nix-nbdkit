{ binutils, lib, stdenv, bash, autoreconfHook, pkg-config, which
, fetchFromGitLab
, runCommand
...
stdenv.mkDerivation ({
  ...
  '' + ''
    ...
    ocamlopt -verbose -S -output-obj -runtime-variant _pic -o conftest.so conftest.ml || { cat conftest.s; ${binutils}/bin/as -o 'conftest.o' 'conftest.s'; }
  '' ; 
